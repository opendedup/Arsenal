...
class VersioningRequestProcessor {
    ...
    put(request, logger, callback) {
        ...
        // - versioning && versionId: create a new version which has
        //   the specified versionId (e.g. for async replication)
    ...
    processNewVersionPut(request, logger, callback) {
        let versionId;
        if (request.options && request.options.versionId) {
            // using specified versionId
            versionId = request.options.versionId;
        } else {
